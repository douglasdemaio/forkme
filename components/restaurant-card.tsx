    ...
    <Link href={`/restaurants/${r.slug}`}
      ...
      {r.banner ? (
        <div className="relative h-32 w-full">
          <Image src={r.banner} alt={r.name} fill className="object-cover" />
        </div>
      ) : (
        <div className="h-32 w-full bg-gradient-to-br from-dark-800 to-dark-700 flex items-center justify-center">
        ...
        <div className="flex items-start gap-3">
          {r.logo && (
            <div className="relative w-10 h-10 rounded-full overflow-hidden flex-shrink-0 -mt-7 ring-2 ring-dark-900">
              <Image src={r.logo} alt="" fill className="object-cover" />
            </div>
          )}
